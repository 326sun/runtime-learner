...
export function readEvents(baseDir = learnerDir(), { limit = 200, type = null, entityId = null } = {}) {
  ...
  const rows = [];
  const lines = fs.readFileSync(file, "utf-8").split("\n").filter(Boolean);
  const sliced = lines.slice(-Math.max(limit * 4, limit));
  const sliceStart = lines.length - sliced.length;
  for (let i = 0; i < sliced.length; i++) {
    try {
      const row = JSON.parse(sliced[i]);
      if (type && row.type !== type) continue;
      if (entityId && row.entityId !== entityId) continue;
      // seq is the log line index: dates only have millisecond precision, so
      // events appended within the same millisecond share an identical date
      // string and replayEventState needs the append order to break the tie.
      rows.push({ ...row, seq: sliceStart + i });
    } catch {}
  }
